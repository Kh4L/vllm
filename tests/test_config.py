...
def test_reconfigure_for_independent_dp_rank_on_replicated_moe_model():
    ...
    assert parallel_config.data_parallel_rank == 0
    assert parallel_config.data_parallel_index == 1
    assert parallel_config.elastic_ep_max_dp_size == 1
...
def test_data_parallel_replicate_moe_invalid_combos(
    ...
    kwargs = {
        ...
        "is_moe_model": True,
        "data_parallel_replicate_moe": True,
        # Pin the executor so the GPU-count check for the PP=2 row does not
        # pre-empt the replicate-mode validation on single-GPU hosts.
        "distributed_executor_backend": "mp",
        **extra_kwargs,
    }
...
@pytest.mark.parametrize(
    "engram_kwargs",
    [{"embedding_across_dp": True}, {"dp_shared_memory": True, "cpu_offload": True}],
)
def test_data_parallel_replicate_moe_rejects_engram_dp_sharing(engram_kwargs):
    """Engram's cross-DP embedding sharding/sharing assumes the DP ranks form
    one group, which independent replicas do not, so it must fail closed."""
    parallel_config = ParallelConfig(
        data_parallel_size=2,
        is_moe_model=True,
        data_parallel_replicate_moe=True,
    )
    with pytest.raises(NotImplementedError, match="data-parallel-replicate-moe"):
        EngramConfig(**engram_kwargs).verify_parallel_config(parallel_config)
